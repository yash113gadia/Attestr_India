...
import { BookOpen, Send, Copy, Check, ChevronDown, ChevronRight, Key, Trash2, Plus, Loader2 } from 'lucide-react';
...
const ENDPOINTS = [
  {
    ...
    path: '/api/v1/verify',
    title: 'Verify Media',
    description: 'Verify a media file against the blockchain ledger. Accepts a base64-encoded file or a public URL.',
    headers: [
...
function ApiKeyManager() {
  ...
  async function handleGenerate(e) {
    ...
    setNewKey(null);
    try {
      const data = await generateApiKey(label || 'Default');
      setNewKey(data.key);
      setLabel('');
      loadKeys();
    } catch {}
    ...
    return (
      <div className="border border-rule rounded-xl p-5 space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-serif text-[20px] text-ink">API Keys</h3>
        </div>
        <p className="text-[13px] text-ink-tertiary">Sign in to generate and manage API keys.</p>
      </div>
    );
  }
  ...
  return (
    <div className="border border-rule rounded-xl p-5 space-y-4">
      ...
      <p className="text-[13px] text-ink-secondary">
        Generate API keys to authenticate with the <span className="font-mono text-ink">/api/v1/register</span> endpoint. Max 5 active keys.
      </p>
      ...
      <form onSubmit={handleGenerate} className="flex gap-2">
        ...
        <button
          type="submit"
          disabled={loading}
          className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-ink text-void text-[13px] font-medium hover:bg-ink/90 disabled:opacity-40 transition"
        >
      ...
      {newKey && (
        <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-lg p-3 space-y-2">
          <p className="text-[12px] text-emerald-400 font-medium">Key generated — copy it now, it won't be shown again.</p>
          <div className="flex items-center gap-2">
            <code className="flex-1 text-[12px] font-mono text-emerald-300 break-all">{newKey}</code>
            <button onClick={copyKey} className="p-1.5 rounded bg-surface-raised/80 border border-rule text-ink-tertiary hover:text-ink transition">
      ...
      {keys.length > 0 && (
        <div className="border border-rule rounded-lg overflow-hidden">
          <table className="w-full text-[12.5px]">
            <thead>
              <tr className="border-b border-rule bg-surface-raised/30">
                <th className="text-left px-3 py-2 font-mono text-ink-tertiary font-normal">Label</th>
                <th className="text-left px-3 py-2 font-mono text-ink-tertiary font-normal">Key</th>
                <th className="text-left px-3 py-2 font-mono text-ink-tertiary font-normal hidden sm:table-cell">Uses</th>
                <th className="text-left px-3 py-2 font-mono text-ink-tertiary font-normal hidden sm:table-cell">Status</th>
            ...
            <tbody>
              {keys.map((k) => (
                <tr key={k.id} className="border-b border-rule last:border-0">
                  <td className="px-3 py-2 font-mono text-ink">{k.label}</td>
                  <td className="px-3 py-2 font-mono text-ink-secondary">{k.keyPreview}</td>
                  <td className="px-3 py-2 text-ink-tertiary hidden sm:table-cell">{k.usageCount}</td>
                  <td className="px-3 py-2 hidden sm:table-cell">
                    <span className={`text-[10px] px-1.5 py-0.5 rounded font-mono ${k.active ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-400'}`}>
                      {k.active ? 'active' : 'revoked'}
                    </span>
                  </td>
                  <td className="px-3 py-2 text-right">
                    {k.active && (
                      <button onClick={() => handleRevoke(k.id)} className="p-1 rounded text-ink-tertiary hover:text-red-400 transition" title="Revoke key">
...
export default function ApiDocsPage() {
  return (
    <motion.div initial="hidden" animate="show" className="max-w-3xl mx-auto space-y-8 pb-16">
      ...
      {/* API Key Manager */}
      <motion.div variants={fade} custom={2.5}>
        <ApiKeyManager />
      </motion.div>

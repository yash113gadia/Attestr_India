...
import { Camera as CameraIcon, ArrowLeft, Shield, Lock, Fingerprint, Blocks, Eye, ChevronDown, ChevronRight, ShieldCheck } from 'lucide-react';
    ...
    <div className="pb-6">
      ...
      <div className="mb-5">
        ...
        <div className="flex items-center gap-2 mt-2">
          ...
          <span className="text-[11px] font-mono text-ink-faint">Immutable Ledger · Privacy-First</span>
        </div>
      </div>
              ...
              {!result && (
                <div className="p-3 border-t border-rule-light">
                  <button onClick={handleRegister} disabled={registering}
                    className="w-full py-3.5 bg-accent text-white text-[14px] font-medium rounded-sm active:scale-[0.98] transition-transform disabled:opacity-40 flex items-center justify-center gap-2">
                    {registering ? (
                      <motion.span animate={{ opacity: [0.4, 1, 0.4] }} transition={{ duration: 1.2, repeat: Infinity }} className="text-[13px]">
                        Anchoring to ledger...
                      </motion.span>
                    ) : (
                      <><Blocks className="w-4 h-4" /> Seal on Chain</>
                    )}
                  </button>
                </div>
              )}
                  ...
                  {getFileCategory(file) === 'image' ? (
                    ...
                    <div className="border border-rule rounded-sm bg-surface p-4">
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <span className="text-[12px] font-medium text-ink">{getFileMeta(file).label} Integrity</span>
                      </div>
                      <p className="text-[11px] text-ink-tertiary leading-relaxed">SHA-256 fingerprint ensures any modification to this file will be detected. Byte-level integrity verification.</p>
                    </div>
                  )}
                </motion.div>
              )}

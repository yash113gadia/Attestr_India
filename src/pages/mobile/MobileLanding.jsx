...
export default function MobileLanding() {
  return (
    <div className="bg-void text-ink min-h-[100dvh] flex flex-col">
      {/* Nav */}
      <nav className="px-5 pt-[env(safe-area-inset-top)] bg-void/80 backdrop-blur-2xl border-b border-rule sticky top-0 z-50">
        <div className="h-12 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <Logo size={18} />
            <span className="font-serif text-[15px] text-ink tracking-tight">Attestr</span>
            <span className="text-[9px] text-kesari font-medium">प्रमाण</span>
          </div>
          <div className="flex items-center gap-4">
            <Link to="/register" className="text-[11px] text-accent font-mono tracking-wider">LAUNCH</Link>
          </div>
        </div>
      </nav>
      ...
      <section className="flex-1 flex flex-col justify-center px-6 py-12">
        ...
        <motion.h1 {...fade(0.1)} className="font-serif text-[44px] leading-[0.92] tracking-tight mb-5">
          Proof of<br />existence<br />forever.
        </motion.h1>

        <motion.p {...fade(0.2)} className="text-[15px] text-ink-secondary leading-relaxed mb-8 max-w-[280px]">
          Anchor any file to the Ethereum blockchain. Establish provenance. Detect tampering.
        </motion.p>
        ...
        <motion.div {...fade(0.4)} className="flex flex-wrap gap-2">
          {['Privacy-first', 'Ethereum Sepolia', 'All file types'].map((t) => (
      ...
      <section className="py-8 px-5 border-t border-rule">
        ...
        <div className="grid grid-cols-2 gap-2">
          {[
            { icon: Lock, title: 'Zero upload', desc: 'Processed locally' },
            { icon: Fingerprint, title: 'Dual hash', desc: 'Exact + fuzzy' },
            { icon: Blocks, title: 'Ethereum', desc: 'Immutable ledger' },
            { icon: Search, title: 'ELA', desc: 'Edit detection' },
            { icon: ShieldCheck, title: 'EXIF', desc: 'Metadata scan' },
            { icon: Camera, title: 'Capture', desc: 'Chain of custody' },
          ].map((f) => (

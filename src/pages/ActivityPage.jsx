    ...
    <div>
      <motion.div initial="hidden" animate="show" className="flex items-start justify-between mb-6 md:mb-8">
        <motion.div variants={fade} custom={0}>
          <h1 className="font-serif text-[28px] md:text-[32px] text-ink leading-none tracking-tight">Ledger</h1>
          <p className="text-[13px] text-ink-tertiary mt-2">Public activity feed. Filenames and identities are redacted.</p>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }} className="mb-6 md:mb-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-px bg-rule rounded-sm overflow-hidden border border-rule">
            ...
            <Stat 
              ...
              color={data.chainValid ? 'text-verified' : 'text-danger'} 
              delay={0.3} 
            />
            <Stat 
              icon={Globe} 
              label="NETWORK" 
              value={data.onChain ? 'Ethereum Sepolia' : 'Local Ledger'} 
              delay={0.4} 
            />
          </div>
